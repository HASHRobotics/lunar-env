...
[Gx1, Gy1] = imgradientxy(heightmap, 'intermediate');
[Gmag1, Gdir1] = imgradient(Gx1,Gy1);
Gmag1 = mat2gray(Gmag1);
BM1 = (Gmag1 >= 0 & Gmag1 < gradient_threshold);
pit_mask = heightmap == -80;
local_occupancy_map1 = ~pit_mask & BM1;
figure
imshow(local_occupancy_map1);

% Gx = Gx1;
% Gy = Gy1;
flipped_image = flip(heightmap, 1);
flipped_image = flip(flipped_image, 2);

[Gx2, Gy2] = imgradientxy(flipped_image, 'intermediate');
Gx2 = flip(Gx2, 1);
Gx2 = flip(Gx2, 2);
Gy2 = flip(Gy2,1);
Gy2 = flip(Gy2,2);
...
BM2 = (Gmag2 >= 0 & Gmag2 < gradient_threshold);
pit_mask = heightmap == -80;
local_occupancy_map2 = ~pit_mask & BM2;
figure
imshow(local_occupancy_map2);

local_occupancy_map = local_occupancy_map1 & local_occupancy_map2;
figure
